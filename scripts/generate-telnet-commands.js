...
const SOURCE = path.join(__dirname, "..", "src", "common", "constants.ts");
const TARGET = path.join(__dirname, "..", "src", "renderer", "telnet-commands.gen.ts");

const source = fs.readFileSync(SOURCE, "utf8");
const match = source.match(/export const TELNET_COMMANDS = \{[\s\S]*?\} as const;/);
if (!match) {
...
const commandCount = (match[0].match(/^\t[a-zA-Z0-9_]+: \{/gm) ?? []).length;
